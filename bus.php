...
if (isset($_GET["checkin"])) {
    $bus_go_check_in_time =   date('Y-m-d H:i:m');
    $bus_go_check_in_by   =   $_SESSION['username'];

    if (count($_POST["bus"]) > 0) {

        $all = implode(",", $_POST["bus"]);

        $query = $db->query("UPDATE tb_member SET bus_go_check_in='Check In',
                                                        bus_go_check_in_time='$bus_go_check_in_time',
                                                        bus_go_check_in_by='$bus_go_check_in_by'
                             WHERE id IN ($all)");
        if ($query) {
            header("Location: ./bus.php?Success");
        } else {
            header("Location: ./bus.php?Failed");
        }
    } else {
        header("Location: ./bus.php?SelectedPlease");
    }
}
...
if (isset($_GET["cencel"])) {
    if (count($_POST["bus"]) > 0) {
        ...
        $query = $db->query("UPDATE tb_member SET bus_go_check_in='Cencel',
                                                        bus_go_check_in_time=NULL,
                                                        bus_go_check_in_by=NULL
                             WHERE id IN ($all)");
        if ($query) {
            header("Location: ./bus.php?SuccessC");
        } else {
            header("Location: ./bus.php?FailedC");
        }
    } else {
        header("Location: ./bus.php?SelectedPlease");
    }
...
?>
<title>Bus G-<?= $Rrole['bus_go']; ?> | Kuehne+Nagel Indonesia - BDG Gathering</title>
<div class="page-contain">
    <!-- Main content -->
    <div id="main-content" class="main-content">
        <!-- Page -->
        <div class="hero-section hero-background-bus style-02" style="margin-top: 49px;">
            <h1 class="page-title">Bus G-<?= $Rrole['bus_go']; ?></h1>
        ...
        <div class="product-related-box single-layout">
            <div class="biolife-title-box lg-margin-bottom-26px-im" style="margin-top: -45px;margin-bottom: -30px;">
                ...
                <div>
                    <img src="assets/icon/bus.png" alt="bus">
                </div>
                <span class="subtitle">Data List Bus G-<?= $Rrole['bus_go']; ?> </span>
                <h3 class="main-title">Bus G-<?= $Rrole['bus_go']; ?></h3>
            </div>
        </div>
                    ...
                    <div class="table-responsive">
                        <table id="C_TableDefault" class="table table-bordered first">
                            ...
                            <tbody>
                                <?php
                                $G         = $Rrole['bus_go'];
                                $dataTable = $db->query("SELECT * FROM tb_member WHERE bus_go='$G' ORDER BY id ASC", 0);
                                if (mysqli_num_rows($dataTable) > 0) {
                                    $no = 0;
                                    while ($row = mysqli_fetch_array($dataTable)) {
                                        $no++;

                                        if ($row['bus_go_check_in'] == NULL) {
                                            $class = 'table-icon';
                                        } else if ($row['bus_go_check_in'] == 'Check In') {
                                            $class = 'table-icon-checkin';
                                        } else if ($row['bus_go_check_in'] == 'Cencel') {
                                        ...
                                        <tr <?= $bg ?>>
                                            <td style="width: 5px;">
                                                <div style="margin-top: 15px;">
                                                    <input type="checkbox" id="chk" name="bus[]" value="<?= $row['id'] ?>">
                                                </div>
                                            </td>
                                            <!-- Hostname & Username -->
                                            <td>
                                                <div style="display: flex;justify-content:flex-start;align-items: center;">
                                                    ...
                                                    <div style="margin-left: 5px;display: grid;text-align: initial;">
                                                        ...
                                                        <div style="font-size: 10px;font-weight: 300;display: flex;justify-content: flex-start;">
                                                            G-<?= $row['bus_go']; ?> /<b><?= $row['bus_go_sheet_n']; ?>-<?= $row['bus_go_sheet_c']; ?></b>
                                                        </div>
                                                    </div>
                                                </div>
                                            </td>
                                            <?php if ($row['bus_go_check_in'] == NULL) { ?>
                                                <td>
                                            ...
                                            <?php } else if ($row['bus_go_check_in'] == 'Check In') { ?>
                                                <td>
                                                    <div style="display: flex;justify-content:flex-start;align-items: center;">
                                                        <div class="<?= $class ?>">
                                                        ...
                                                        <div style="margin-left: 5px;display: grid;text-align: initial;">
                                                            ...
                                                            <div style="font-size: 10px;font-weight: 300;display: flex;justify-content: flex-start;">
                                                                <?php
                                                                $alldate = $row['bus_go_check_in_time'];
                                                                $tgl = substr($alldate, 0, 10);
                                                                $time = substr($alldate, 10, 20);
                                                                ?>
                                                                <?= $time; ?><br><?= $row['bus_go_check_in_by']; ?>
                                                            </div>
                                                        </div>
                                                    </div>
                                                </td>
                                            <?php } else if ($row['bus_go_check_in'] == 'Cencel') { ?>
                                                <td>
                                                    <div style="display: flex;justify-content:flex-start;align-items: center;" class="blink">
...
<script type="text/javascript">
    $("#btn-checkin-yes").click(function() {
        $("#form-submit").attr('action', `bus.php?checkin`)
    ...
    $("#btn-checkin-cencel").click(function() {
        $("#form-submit").attr('action', `bus.php?cencel`)
